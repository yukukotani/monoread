...
describe("Content extraction fallback integration", () => {
  ...
  it("should return readability result when successful", async () => {
    // readabilityが成功する場合
    const { extractContentByReadability } = await import("../libs/readability.js");
    const mockExtractContentByReadability = vi.mocked(extractContentByReadability);
  ...
  it("should fallback to llms.txt when readability fails (empty content)", async () => {
    ...
    const { isReadabilityResultEmpty, extractContentFromLlmsTxt } = await import("../libs/llms-txt.js");
    const mockIsReadabilityResultEmpty = vi.mocked(isReadabilityResultEmpty);
    ...
    assert(result.success);
    if (result.success) {
      assert(result.content === "# LLMS.txt Content\\n\\nThis is from llms.txt fallback.");
      assert(result.metadata?.source === "https://example.com/page");
  ...
  it("should fallback to llms.txt when readability throws exception", async () => {
    ...
    mockExtractContentByReadability.mockRejectedValue(new Error("Readability parsing failed"));
    mockExtractContentFromLlmsTxt.mockResolvedValue({
      success: true,
      content: "# LLMS.txt Fallback\\n\\nThis content was retrieved after readability failed.",
      metadata: {
        source: "https://example.com/page",
        fileType: "llms-txt",
      },
    ...
    assert(result.success);
    if (result.success) {
      assert(result.content === "# LLMS.txt Fallback\\n\\nThis content was retrieved after readability failed.");
      assert(result.metadata?.source === "https://example.com/page");
      assert(result.metadata?.fileType === "llms-txt");
    }
  });
  ...
  it("should fallback to llms.txt when fetch fails", async () => {
    ...
    assert(result.success);
    if (result.success) {
      assert(result.content === "# LLMS.txt Success\\n\\nContent from llms.txt when page is not found.");
